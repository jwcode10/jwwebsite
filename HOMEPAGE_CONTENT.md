# Journey Well - Homepage Content Structure

## Document Purpose
This document contains the complete SEO-optimized content structure for the Journey Well homepage. **Reference this document when building the homepage or when creating content that should match the homepage tone and style.**

---

## SEO Metadata

### SEO Title
```
Journey Well | Content Agency in Baton Rouge
```

### Meta Description
```
Full-service content agency in Baton Rouge for podcasts, YouTube, reels, and social media; strategy, production, and publishing.
```

### Focus Keyword
```
Journey Well content agency Baton Rouge
```

### Supporting Keywords
- content creation agency Baton Rouge
- content production Baton Rouge
- creative agency Baton Rouge
- podcast production Baton Rouge
- YouTube editing Baton Rouge
- short-form video Baton Rouge
- social media management Baton Rouge

---

## Hero Section

### Headline (H1)
```
Journey Well — Content Agency in Baton Rouge, LA
```

### Subheadline
```
Helping creators and brands in Baton Rouge produce high-performing podcasts, videos, and social content without the guesswork. Journey Well brings strategy, production, and publishing under one roof so you can focus on your message while we handle the rest.
```

### Primary CTA
```
Start your content journey → Get a Free Consultation
```

---

## What We Do Section

### Section Title (H2)
```
What We Do
```

### Content
```
Journey Well is a full-service content creation agency in Baton Rouge built for busy founders, marketers, and creators. From content planning to editing to posting, our team delivers consistent output and measurable results. Whether you need a launch-ready podcast, an optimized YouTube channel, a batch of on-brand reels, or hands-off social media management, we've got you covered.

We also partner with clients in New York City and surrounding markets, giving your brand the polish of a larger creative agency with the access and agility of a Baton Rouge team.
```

---

## Our Content Services Section

### Section Title (H2)
```
Our Content Services
```

### Service 1: Podcast Production
**Title**: Podcast Production

**Description**:
```
Plan, record, edit, and publish end to end. We offer single-episode, 2-episode, and 4-episode batches as well as monthly packages for 3–4 episodes. Expect clean audio, professional mixing, show notes, and platform publishing so your message reaches Apple Podcasts, Spotify, and beyond.
```

**Link**: `/services/podcast-production-baton-rouge/`

---

### Service 2: YouTube Production & Optimization
**Title**: YouTube Production & Optimization

**Description**:
```
Turn raw footage into long-form episodes that retain viewers and grow subscribers. We handle editing, thumbnail design, titles, descriptions, chapters, and upload optimization. We'll also create evergreen clips to extend the life of every upload.
```

**Link**: `/services/youtube-production-baton-rouge/`

---

### Service 3: Short-Form Video (Reels & Clips)
**Title**: Short-Form Video (Reels & Clips)

**Description**:
```
Stay consistent with batching sessions that produce 6–25+ edited reels per month. Our team trims, captions, formats, and sequences clips for Instagram, TikTok, LinkedIn, and YouTube Shorts then schedules them for you.
```

**Link**: `/services/short-form-video-baton-rouge/`

---

### Service 4: Social Media Management
**Title**: Social Media Management

**Description**:
```
Hands-off social media management in Baton Rouge across two to three platforms. We build content calendars, write captions, schedule posts, and monitor analytics so you publish with intention; not panic.
```

**Link**: `/services/social-media-management-baton-rouge/`

---

### Service 5: Branding & Creative Design
**Title**: Branding & Creative Design

**Description**:
```
Tie your visual identity together with branded carousels, YouTube banners, thumbnail templates, and a cohesive brand suite. Consistency compounds; our creative team ensures your content looks unmistakably "you."
```

**Link**: `/services/branding-and-creative-baton-rouge/`

---

### Service 6: Strategy, Planning & Analytics
**Title**: Strategy, Planning & Analytics

**Description**:
```
Start with clear goals, then track what works. We run strategy calls, map content pillars, and deliver quarterly analytics deep dives on higher tiers. You'll also get a managed link & offer portal so your audience can act quickly when they're ready.
```

**Link**: `/services/content-strategy-baton-rouge/`

---

### Services Section CTA
```
See packages & pricing → Explore Content Plans
```

**Link**: `/packages/`

---

## Why Brands Trust Journey Well Section

### Section Title (H2)
```
Why Brands Trust Journey Well
```

### Benefit 1
**Title**: All-in-one content production

**Description**: Strategy, recording, editing, and publishing—from one Baton Rouge partner.

### Benefit 2
**Title**: Proven, repeatable process

**Description**: We turn your ideas into a reliable content engine that ships on schedule.

### Benefit 3
**Title**: Optimization built-in

**Description**: From YouTube thumbnails to podcast show notes, every piece is created to perform.

### Benefit 4
**Title**: Local access, national polish

**Description**: Baton Rouge roots, NYC experience fast communication and agency-level quality.

### Benefit 5
**Title**: Data-driven iteration

**Description**: Quarterly analytics reviews guide what to double down on and what to refine.

### Section CTA
```
Book a strategy call → Start Planning
```

---

## How We Work Section

### Section Title (H2)
```
How We Work (Simple, Repeatable, Effective)
```

### Step 1: Plan
**Title**: Plan

**Description**:
```
We align on audience, topics, and outcomes. You'll leave with a content roadmap, shoot checklist, and publishing calendar tailored to your goals.
```

### Step 2: Produce
**Title**: Produce

**Description**:
```
Studio or remote, we capture clean audio and video, then edit for clarity and engagement. Our editors add captions, hooks, pattern interrupts, and on-brand graphics.
```

### Step 3: Publish
**Title**: Publish

**Description**:
```
We upload across your chosen platforms like YouTube, Spotify/Apple Podcasts, Instagram, LinkedIn, TikTok and we are also using best-practice metadata and scheduling tools.
```

### Step 4: Perform
**Title**: Perform

**Description**:
```
Quarterly analytics synthesize watch time, retention, CTR, reach, and conversions. We iterate your content pillars and hooks so every cycle performs better than the last.
```

### Section CTA
```
Make my content consistent → Get Started
```

---

## Results That Matter Section

### Section Title (H2)
```
Results That Matter
```

### Introduction
```
Our clients across Baton Rouge and NYC use Journey Well to:
```

### Result 1
Launch shows that publish on time, every time.

### Result 2
Grow YouTube channels with better retention and click-through.

### Result 3
Multiply long-form content into weeks of short-form clips.

### Result 4
Maintain a steady presence on Instagram, LinkedIn, and YouTube without hiring internally.

### Section CTA
```
See what's possible: Portfolio Link
```

**Link**: `/portfolio/`

---

## Industries We Serve Section

### Section Title (H2)
```
Industries We Serve
```

### Content
```
Coaches & consultants, professional services, local businesses, healthcare, real estate, SaaS, financial services, and community organizations. If your audience learns from content, we can help you produce it consistently.
```

---

## Serving Baton Rouge & Beyond Section

### Section Title (H2)
```
Serving Baton Rouge & Beyond
```

### Content
```
Journey Well is proud to be a content agency in Baton Rouge, serving the metro area and nearby communities such as Gonzales, Prairieville, Denham Springs, and Central. We also collaborate with teams in New York City for bi-coastal production schedules and remote studio support.
```

### NAP Information (for Local SEO)
```
Journey Well — Content Agency, Baton Rouge, LA
Phone: (add number)
Email: (add email)
```

### Section CTA
```
Contact Us
```

**Link**: `/contact/`

---

## FAQs Section

### Section Title (H2)
```
FAQs
```

### FAQ 1
**Question**: What makes Journey Well different from a traditional creative agency?

**Answer**: We're a content production partner first built around shipping weekly content that compounds. Strategy, production, and publishing live together so you don't manage multiple vendors.

### FAQ 2
**Question**: Can you work with our existing brand team?

**Answer**: Absolutely. We can operate as your external production arm or integrate with in-house designers and marketers.

### FAQ 3
**Question**: Do you provide scripts or prompts for founders?

**Answer**: Yes. We prepare outlines, talking points, and questions to keep recordings clear, confident, and on-brand.

### FAQ 4
**Question**: How long does it take to launch a podcast or YouTube series?

**Answer**: Most clients launch within 3–6 weeks depending on scope, assets, and scheduling.

### FAQ 5
**Question**: Do you handle posting and analytics?

**Answer**: Yes. On growth tiers and up, we schedule posts and provide quarterly analytics to guide continuous improvement.

---

## Final CTA Section

### Section Title (H2)
```
Ready to Start Your Content Journey?
```

### Content
```
Whether you're launching a show, optimizing your YouTube channel, or scaling short-form content, Journey Well gives you the team, tools, and process to create with confidence.
```

### Primary CTA
```
Get a Free Consultation
```

### Secondary CTA
```
Explore Packages
```

**Link**: `/packages/`

---

## Content Tone & Style Guidelines

### Voice
- Professional yet approachable
- Confident but not boastful
- Clear and direct
- Solution-focused

### Writing Style
- Use active voice
- Short, scannable paragraphs
- Bullet points for lists
- Clear value propositions
- Action-oriented CTAs

### Key Messages
- All-in-one solution
- Consistent, reliable output
- Data-driven approach
- Local roots, national quality
- Process-driven methodology

---

## Internal Linking Requirements

### Required Links from Homepage:
- All 6 service pages (in "Our Content Services" section)
- `/packages/` (multiple CTAs)
- `/contact/` (in NAP section and final CTA)
- `/portfolio/` (in "Results That Matter" section)
- `/locations/baton-rouge/` (in "Serving Baton Rouge" section)

---

## Notes for Future Agents

1. **Use this content structure** when building or updating the homepage
2. **Maintain the tone and style** in all content
3. **Ensure all internal links** are included as specified
4. **Keep SEO keywords** naturally integrated (don't keyword stuff)
5. **Reference `DESIGN_PRD.md`** for visual presentation
6. **Update NAP information** when contact details are finalized
7. **Keep CTAs action-oriented** and clear

---

**Last Updated**: [Current Date]
**Maintained By**: Development Team

